...
class DockerContainer(protected val id: ContainerId,
  ...
  override def suspend()(implicit transid: TransactionId): Future[Unit] = {
    super.suspend().flatMap(_ => docker.pause(id))
  }
  override def resume()(implicit transid: TransactionId): Future[Unit] = {
    docker.unpause(id).flatMap(_ => super.resume())
  }
  ...
  /** Inspect the memory of container via cgroups memory file */
  override def catMemory()(implicit transid: TransactionId): Double = {
    if (useRunc) runc.catMemory(id) else docker.catMemory(id)
  }
}
