...
class ContainerProxy(factory: (TransactionId,
  ...
  when(Starting) {
    ...
    case Event(completed: PreWarmCompleted, _) =>
      ...
      recorder.setStart(Instant.now)    // 预热容器开始统计内存浪费时间
      //      goto(Started) using completed.data    原来的
      goto(Paused) using completed.data
    ...
    case Event(RunCompleted, data: WarmedData) =>
      ...
      if (requestWork(data) || activeCount > 0) {
        ...
        if (!data.action.name.name.contains("invokerHealthTest")) {
          recorder.setStart(Instant.now)    //保活容器开始计算内存浪费时间
          println(s"【warmed container】 ${data.container.containerId}, start keep-alive")
        }
  ...
  when(Ready, stateTimeout = pauseGrace) {    //pauseGrace：用于很短暂的暂停，以防在很短时间内，又来了新的job: Run
    case Event(job: Run, data: WarmedData) =>
      implicit val transid = job.msg.transid
      activeCount += 1
      if (!data.action.name.name.contains("invokerHealthTest")) {
        recorder.setEnd(Instant.now)
        recorder.summary(data.container.catMemory()(TransactionId.invokerNanny), "hit")
        println(s"【warmed container】 ${data.container.containerId} hit in 【Ready】 state, stop recorder")
      }
    ...
    case Event(Remove, data: WarmedData) =>
      if (!data.action.name.name.contains("invokerHealthTest")) {
        recorder.setEnd(Instant.now)
        recorder.summary(data.container.catMemory()(TransactionId.invokerNanny), "dead")
        println(s"【warmed container】 ${data.container.containerId} dead without hit in 【Ready】 state recevie【Remove】, stop recorder")
      }
    ...
    case Event(_: FailureMessage, data: WarmedData) =>
      if (!data.action.name.name.contains("invokerHealthTest")) {
        recorder.setEnd(Instant.now)
        recorder.summary(data.container.catMemory()(TransactionId.invokerNanny), "dead")
        println(s"【warmed container】 ${data.container.containerId} dead without hit in 【Ready】 state receive【FailureMessage】, stop recorder")
      }
      destroyContainer(data, true, wastedMemoryTime = recorder.timeline)
  }
  ...
  when(Pausing) {
    ...
    case Event(_: FailureMessage, data: WarmedData) =>
      if (!data.action.name.name.contains("invokerHealthTest")) {
        recorder.setEnd(Instant.now)
        recorder.summary(data.container.catMemory()(TransactionId.invokerNanny), "dead")
        println(s"【warmed container】 ${data.container.containerId} dead without hit in 【Pausing】 state receive【FailureMessage】, stop recorder")
      }
      destroyContainer(data, true, wastedMemoryTime = recorder.timeline)
    case _                                          => delay
  }
  ...
  when(Paused, stateTimeout = unusedTimeout) {
    case Event(job: Run, data: WarmedData) =>     //针对Warmed容器【原有机制，没有改】
      implicit val transid = job.msg.transid
      if (!data.action.name.name.contains("invokerHealthTest")) {
        recorder.setEnd(Instant.now)
        recorder.summary(data.container.catMemory()(TransactionId.invokerNanny), "hit")//这段时间存在内存浪费，但是hit
        println(s"【warmed container】 ${data.container.containerId} hit in 【Paused】 state receive【Run】, stop recorder")
      }
      activeCount += 1
      val newData = data.withResumeRun(job)
      data.container
    ...
    case Event(job: Run, data: PreWarmedData) =>   //针对PreWarmed容器。
      implicit val transid = job.msg.transid
      if (!job.action.name.name.contains("invokerHealthTest")) {
        recorder.setEnd(Instant.now)
        recorder.summary(data.container.catMemory()(TransactionId.invokerNanny), "hit")//这段时间存在内存浪费，但是hit
        println(s"【prewarmed container】 ${data.container.containerId} hit in 【Paused】 state receive【Run】, stop recorder")
      }
      logging.info(
        this,
        s" This prewarmed Container just receives a running job. ContainerID: ${data.container.containerId}")
      activeCount += 1
      initializeAndRun(data.container, job)
        .map(_ => RunCompleted)
        .pipeTo(self)
      goto(Running) using PreWarmedData(data.container, data.kind, data.memoryLimit, 1, data.expires) //data.expires就是距离ddl还有多久
    ...
    case Event(StateTimeout | Remove, data: WarmedData) =>
      if (!data.action.name.name.contains("invokerHealthTest")) {
        recorder.setEnd(Instant.now)
        recorder.summary(data.container.catMemory()(TransactionId.invokerNanny), "dead")
        println(s"【warmed container】 ${data.container.containerId} dead without hit in 【Paused】 state receive【StateTimeout | Remove】, stop recorder")
      }
    ...
    case Event(StateTimeout, data: PreWarmedData) =>
      //rescheduleJob = true // to suppress sending message to the pool and not double count
      //NumofPre.PreMAP("GO_Action3") = (0, 0)  //没法得到pre-warm容器的action信息，属实没办法了，先这么凑合着
      //获取PreMap中全部action的map，如果哪个action的get._2 ==1，就修改它，把对应的容器数量改为0，并把状态码改为0

      recorder.setEnd(Instant.now)
      recorder.summary(data.container.catMemory()(TransactionId.invokerNanny), "dead")
      println(s"【prewarmed container】 ${data.container.containerId} dead without hit in 【Paused】 state receive【StateTimeout】, stop recorder")
      println(s"This PreWarmed Container is too old to keep alive. ContainerID: ${data.container.containerId}. It has been kept for ${unusedTimeout} minutes")
      destroyContainer(data, false, wastedMemoryTime = recorder.timeline)
  }
  ...
  when(Removing) {
    ...
    case Event(_: FailureMessage, data: WarmedData) =>
      ...
      if (activeCount == 0) {
        ...
        destroyContainer(newData, true, wastedMemoryTime = recorder.timeline)
      } else {
        stay using newData
      }
  }
  ...
  def destroyContainer(newData: ContainerStarted,
    ...
    wastedMemoryTime match {
      case Some(s) => redisClient.setWastedMemoryTime(container.containerId.asString, s, recorder.total)
      case None =>
    }
    if (!rescheduleJob) {
    ...
    val unpause = stateName match {
      // case Paused => container.resume()(TransactionId.invokerNanny)    //这里很关键，需要因为将预热容器统一转移到Paused状态，但实际Docker容器并没有Paused，调用resume会失败
