...
class DockerClient(dockerHost: Option[String] = None,
  ...
  def catMemory(id: ContainerId)(implicit transid: TransactionId): Double = {
    val cat: Seq[String] = Seq("cat")
    //    val cmd = cat ++ Seq("/sys/fs/cgroup/memory/docker/" + id.asString + "/memory.usage_in_bytes")  cgroup-v1
    val cmd = cat ++ Seq("/sys/fs/cgroup/system.slice/docker-" + id.asString + ".scope/memory.current")  // cgroup-v2
    val timeout = config.timeouts.update

    val start = transid.started(
      this,
      LoggingMarkers.INVOKER_RUNC_CMD(cat.head),
      s"running ${cmd.mkString(" ")} (timeout: $timeout)",
      logLevel = InfoLevel)
    val memory = executeProcess(cmd, timeout).andThen {
      case Success(_) => transid.finished(this, start)
      case Failure(pte: ProcessTimeoutException) =>
        transid.failed(this, start, pte.getMessage, ErrorLevel)
        MetricEmitter.emitCounterMetric(LoggingMarkers.INVOKER_DOCKER_CMD_TIMEOUT(cat.head))
      case Failure(t) => transid.failed(this, start, t.getMessage, ErrorLevel)
    }.map(_.toDouble)
    Await.result(memory, timeout)
  }
}
...
trait DockerApi {
  ...
  /**
   * Inspect the memory of container from cgroups memory file
   *
   * @param id the id of the container to remove
   * @return a Future with memory Double value
   */
  def catMemory(id: ContainerId)(implicit transid: TransactionId): Double
}
