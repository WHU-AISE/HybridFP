...
case object RefreshPredictions  // Refresh prediction data from Redis every 60 minutes

case object PredictivePrewarm   // Perform predictive prewarming every minute

// Cache entry for action information: (ExecutableWhiskAction, lastSeen timestamp in minutes)
case class ActionCacheEntry(action: ExecutableWhiskAction, lastSeen: Int)
...
class ContainerPool(childFactory: ActorRefFactory => ActorRef,
  ...
  // Initialize RedisClient for predictive prewarming
  private val redisClient = new RedisClient(logging = logging)
  redisClient.init

  // Learning cache: namespace/action -> ActionCacheEntry
  // This cache is built from actual Run messages to learn action configurations
  var actionCache = immutable.Map.empty[String, ActionCacheEntry]

  // Prediction data storage: funcId -> Array[60 prediction values]
  var predictionData = immutable.Map.empty[String, Array[Int]]

  // Current minute index in the prediction array (0-59)
  // Increments every minute, resets to 0 every hour
  var currentMinuteIndex: Int = 0

  // Schedule 1: Refresh predictions from Redis every 60 minutes
  context.system.scheduler.scheduleAtFixedRate(
    7.minutes,  // start immediately
    60.minutes, // refresh every hour
    self,
    RefreshPredictions
  )

  // Schedule 2: Perform predictive prewarming every minute
  context.system.scheduler.scheduleAtFixedRate(
    5.minutes,   // initial delay to allow first refresh
    60.seconds,   // interval
    self,
    PredictivePrewarm
  )
  ...
  def receive: Receive = {
    ...
    case r: Run =>
      // Learn action information for predictive prewarming
      val funcId = s"${r.msg.user.namespace.name.asString}/${r.action.name.name}"
      val lastSeen = System.currentTimeMillis()/60000
      actionCache = actionCache + (funcId -> ActionCacheEntry(r.action, (System.currentTimeMillis()/60000).toInt))
      // Continue with normal Run processing
    ...
    case RefreshPredictions =>
      // Refresh prediction data from Redis every 60 minutes
      logging.info(this, "[Predictive Prewarming] Refreshing predictions from Redis")
      predictionData = redisClient.getPredictions()
      currentMinuteIndex = 0  // Reset minute index
      logging.info(this, s"[Predictive Prewarming] Loaded ${predictionData.size} prediction arrays, reset index to 0")

    case PredictivePrewarm =>
      // Use current minute's prediction value
      if (predictionData.nonEmpty) {
        logging.info(this, s"[Predictive Prewarming] Processing minute index $currentMinuteIndex")
        handlePredictivePrewarm(predictionData, currentMinuteIndex)

        // Increment minute index, wrap around at 60
        currentMinuteIndex = (currentMinuteIndex + 1) % 60
      } else {
        logging.info(this, "[Predictive Prewarming] No prediction data available, skipping")
      }
  }
  ...
  /**
   * Handle predictive prewarming based on predictions from Redis.
   * This method checks existing idle containers and creates new prewarm containers if needed.
   *
   * @param predictions Map of funcId (namespace/action) to array of 60 prediction values
   * @param minuteIndex Current minute index (0-59) to use from the prediction array
   */
  def handlePredictivePrewarm(predictions: Map[String, Array[Int]], minuteIndex: Int): Unit = {
    if (predictions.isEmpty) {
      return
    }

    logging.info(this, s"[Predictive Prewarming] Processing ${predictions.size} predictions for minute index $minuteIndex")

    predictions.foreach { case (funcId, predictionArray) =>
      // Get prediction value for current minute
      val predictedCount = if (minuteIndex >= 0 && minuteIndex < predictionArray.length) {
        predictionArray(minuteIndex)
      } else {
        logging.info(this, s"[Predictive Prewarming] Invalid minute index $minuteIndex for $funcId")
        0
      }

      // Skip if prediction is 0 or negative
      if (predictedCount > 0) {
        actionCache.get(funcId) match {
          case Some(cacheEntry) =>
            val action = cacheEntry.action
            val kind = action.exec.kind
            val memory = action.limits.memory.megabytes.MB
            val namespace = action.namespace.namespace
            val actionName = action.name.name

            // Count existing idle containers for this specific action
            val existingWarmedIdleCount = freePool.count { case (_, data) =>
              data match {
                case WarmedData(_, ns, act, _, 0, _)
                  if ns.asString == namespace && act.name.name == actionName => true
                case _ => false
              }
            }

            // Count existing prewarm containers
            val existingPrewarmCount = prewarmedPool.count { case (_, data) =>
              data.kind == kind && data.memoryLimit == memory
            }
            val startingPrewarmCount = prewarmStartingPool.count { case (_, (k, m)) =>
              k == kind && m == memory
            }
            val totalExistingCount = existingWarmedIdleCount + existingPrewarmCount + startingPrewarmCount

            // Calculate how many containers we need to create
            val neededCount = Math.max(0, Math.ceil(predictedCount.toDouble / 10).toInt - totalExistingCount)

            if (neededCount > 0) {
              println(s"[Predictive Prewarming] funcId=$funcId, predicted=$predictedCount, " +
                s"existing(warmed=$existingWarmedIdleCount, prewarm=$existingPrewarmCount, starting=$startingPrewarmCount), " +
                s"creating=$neededCount prewarm containers (kind=$kind, memory=${memory.toMB}MB)")

              // Create prewarm containers
              (1 to neededCount).foreach { i =>
                try {
                  prewarmContainer(action.exec, memory, None)
                } catch {
                  case e: Exception =>
                    logging.error(this, s"[Predictive Prewarming] Failed to create prewarm container for $funcId: ${e.getMessage}")
                }
              }
            } else {
              println(
                s"[Predictive Prewarming] funcId=$funcId, predicted=$predictedCount, needed=$neededCount, " +
                  s"existing=$totalExistingCount (sufficient), no action needed")
            }

          case None =>
            // Action not in cache yet - will be learned when first execution happens
            println(s"[Predictive Prewarming] funcId=$funcId not in cache yet, will be learned on first execution")
        }        
      }
    }
  }
  ...
  def takePrewarmContainer(action: ExecutableWhiskAction): Option[(ActorRef, ContainerData)] = {
    ...
    logging.info(this, s"Looking for prewarm container: kind=$kind, memory=$memory, Available prewarm containers: ${prewarmedPool.map(p => (p._2.kind, p._2.memoryLimit))}")
    prewarmedPool.toSeq
      ...
      .map {
        case (ref, data) =>
          ...
          logging.info(this, s"Found and using prewarm container: kind=$kind, memory=$memory, actor=$ref")          
          freePool = freePool + (ref -> data)
